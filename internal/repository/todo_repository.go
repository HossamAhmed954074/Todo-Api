...
func (r *TodoRepository) CreateTodo(ctx context.Context, todo *models.Todo) error {
	...
	err := r.db.QueryRow(ctx, query, todo.Title, todo.Description, todo.Completed, todo.CreatedAt, todo.UpdatedAt, todo.IsDeleted).Scan(&todo.ID)
	return err
}
...
func (r *TodoRepository) DeleteTodoById(ctx context.Context, id int) error {
	if exists, err := r.IsTodoExists(ctx, id); err != nil {
		return err
	} else if !exists {
		return nil // No error, but nothing to delete
	}
	query := `UPDATE todos SET is_deleted = true, deleted_at = $1 WHERE id = $2`
